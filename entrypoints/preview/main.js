...
function injectLibsAndTheme(html, theme) {
  const base = browser.runtime.getURL('/preview/lib/');
...
function setTheme(theme) {
  ...
  browser.storage.local.set({ previewTheme: theme });
...
async function init() {
  ...
  try {
    ...
    const result = await new Promise((resolve, reject) => {
      browser.storage.session.get([key, 'autoPrint', 'autoDownloadPng'], (data) => {
    ...
    const prefs = await new Promise((r) => browser.storage.local.get('previewTheme', r));
