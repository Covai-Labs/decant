...
async function main() {
  ...
  if (fs.existsSync(supportedMdPath)) {
    const currentSupported = fs.readFileSync(supportedMdPath, "utf-8");
  ...
  if (fs.existsSync(path.dirname(webDocsPlatformsPath))) {
...
`;
    ...
    const currentTs = fs.existsSync(webDocsPlatformsPath)
      ? fs.readFileSync(webDocsPlatformsPath, "utf-8")
      : "";
